    ...
    <div className="bg-white text-black font-sans text-[10px] leading-tight border border-black/80 print:border-0 max-w-[8.5in] mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mb-3 text-[10px]">
        <div>
          ...
          <div>{bol.shipper.city}, {bol.shipper.state} {bol.shipper.zip}</div>
        </div>
        <div>
          ...
          <div>{bol.consignee.city}, {bol.consignee.state} {bol.consignee.zip}</div>
        </div>
      </div>

      {/* Order Details */}
      <div className="grid grid-cols-3 gap-2 mb-3 text-[10px] border-y border-black/80 py-2">
        <div><span className="text-black/60">BOL #:</span> <span className="font-mono">{bol.bolNumber}</span></div>
        <div><span className="text-black/60">PRO #:</span> <span className="font-mono">{bol.proNumber}</span></div>
        <div><span className="text-black/60">Carrier:</span> {bol.carrier}</div>
      </div>
        ...
        <tbody>
          {bol.lines.map((l, i) => (
            <tr key={i} className="border-t border-black/30">
              <Td mono>{l.sku}</Td>
              <Td>{l.description}</Td>
              <Td className="text-right tabular-nums">{l.qty} {l.pkgType}</Td>
      ...
      <div className="border-t border-black/80 pt-2">
        <div className="text-[8px] uppercase tracking-wider text-black/60 mb-1">Special Instructions</div>
